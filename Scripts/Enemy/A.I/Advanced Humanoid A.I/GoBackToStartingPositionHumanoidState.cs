...
namespace AG
{
    public class GoBackToStartingPositionHumanoidState : State
    {
        public CombatStanceStateHumanoid combatStanceStateHumanoid;
        public IdleStateHumanoid idleState;
        public PatrolStateHumanoid patrolState;
        ...
        public override State Tick(EnemyManager enemy)
        {
            ...
            {
                enemy.startingPosition = patrolState.currentPatrolDestination.position;
            }
            float distanceFromTarget = Vector3.Distance(enemy.startingPosition, enemy.transform.position);
        ...
        {
            if (enemyManager.startingPosition == null) { return; }

            //Rotate manually
            if (enemyManager.isPerformingAction)
            {
                Vector3 direction = enemyManager.startingPosition - enemyManager.transform.position; //Maybe delete enemymanager.?
                direction.y = 0;
                direction.Normalize();
            ...
            else
            {
                ...
                enemyManager.navMeshAgent.SetDestination(enemyManager.startingPosition);
